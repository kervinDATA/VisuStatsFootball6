...
// Route pour récupérer les saisons et les données nécessaires à l'analyse
...
// Route pour récupérer les statistiques de possession et de classement
...
// Route pour récupérer les statistiques d'attaques dangereuses, buts et classement
router.get('/dangerous-attacks-stats', async (req, res) => {
  const { season_name } = req.query;
...
// Route pour récupérer la répartition des buts par intervalle de temps pour deux équipes spécifiques
router.get('/goals-interval-distribution', async (req, res) => {
  const { season_name } = req.query;
  ...
  try {
    ...
    console.error('Erreur lors de la récupération de la répartition des buts :', error.stack);
    res.status(500).json({ message: 'Erreur serveur', details: error.message });
  }
});
